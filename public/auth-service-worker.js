(() => {
  ...
  function extractQuerystring(url) {
    ...
    return url.substring(
      queryStart,
      fragmentStart > 0 ? fragmentStart : void 0
    );
  }
  ...
  function registerVersion(libraryKeyOrName, version5, variant) {
    ...
    if (libraryMismatch || versionMismatch) {
      ...
      if (versionMismatch) {
        warning.push(
          `version name "${version5}" contains illegal characters (whitespace or "/")`
        );
      }
  ...
  var PersistenceUserManager = class _PersistenceUserManager {
    constructor(persistence, auth, userKey) {
      ...
      this.fullUserKey = _persistenceKeyName(
        this.userKey,
        config.apiKey,
        name5
      );
  ...
  var PasswordPolicyImpl = class {
    ...
    validatePassword(password) {
      ...
      status.isValid &&
        (status.isValid =
          (_f = status.containsNonAlphanumericCharacter) !== null &&
          _f !== void 0
            ? _f
            : true);
      return status;
  ...
  var AuthImpl = class {
    ...
    async _getAppCheckToken() {
      ...
      const appCheckTokenResult = await ((_a =
        this.appCheckServiceProvider.getImmediate({ optional: true })) ===
        null || _a === void 0
        ? void 0
        : _a.getToken());
      if (
  ...
  var PhoneAuthCredential = class _PhoneAuthCredential extends AuthCredential {
    ...
    /** @internal */
    _getReauthenticationResolver(auth) {
      return verifyPhoneNumberForExisting(
        auth,
  ...
  var BrowserLocalPersistence = class extends BrowserPersistenceClass {
    constructor() {
      ...
      this.boundEventHandler = (event, poll) =>
        this.onStorageEvent(event, poll);
      this.listeners = {};
      this.localCache = {};
      this.pollTimer = null;
  ...
  async function _getRedirectUrl(
    ...
    const appCheckTokenFragment = appCheckToken
      ? `#${FIREBASE_APP_CHECK_FRAGMENT_ID}=${encodeURIComponent(
          appCheckToken
        )}`
      : "";
